...
@section('page_heading')
    <div class="row">
        <h4 class="page-title">{{ trans('general.agent')}} {{trans('general.contact')}} {{trans('general.list') }}</h4>
    </div>
@endsection
@section('content')
                ...
                <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0"
                    ...
                    @foreach ($contacts as $contact)
                        <tr>
                            ...
                            <td>
                                <a href="phone:{{$contact->primaryPhone->number or '-'}}">{{$contact->primaryPhone->number or '-'}}
                                    <i class="fa fa-phone"></i></a></td>
                            <td>{{ $contact->office }}</td>
                            <td>{{ $contact->address }}</td>
                            <td>{{ $contact->state->name or '-' }}</td>
                            <td>{{ $contact->city->name or '-' }}</td>
