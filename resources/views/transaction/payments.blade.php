...
@section('title')
    {{ trans('general.transaction') }} #{{ $transaction->id }} {{ trans('payment.list') }}
@endsection
@section('page_heading')
    <div class="row">
        <h4 class="page-title">{{ trans('general.transaction') }} #{{ $transaction->id }} {{ trans('payment.list') }}</h4>
    </div>
@endsection
@section('content')
            ...
            <div class="card-box">
                ...
                <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0"
                    ...
                    <tr>
                        ...
                        @if ($transaction->status->rawname == 'cleared')
                        <th>{{ trans('transaction.paid_amount') }}</th>
                        <th>{{ trans('transaction.paid_date') }}</th>
                        @elseif (in_array($transaction->status->rawname, ['accepted', 'pending']) && Auth::user()->can('approve_transaction'))
